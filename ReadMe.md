...
As this was time limited the solution is simply with an attempt to make the rules readable for future developers.

Test are provided using a Test class in `test_sort.py` using pytest.

Assume that Python 3 is installed and working. There a no dependencies other then pytest which can be installed gloabally using `pip install -U` pytest`
